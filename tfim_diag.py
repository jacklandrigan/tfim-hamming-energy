...
def main():
    ...
    parser.add_argument('--h_log', type=bool, default=False,
                            help='Use logarithmic steps for h')
    parser.add_argument('--h_log_num', type=int, default=5,
                            help='number of logarithmic steps')
    parser.add_argument('-J', type=float, default=1.0,
                            help='Nearest neighbor Ising coupling')
    parser.add_argument('-S', type=int, default=0,
                            help='seed for Jij matrix')
    ...
    # Transverse field
    if args.h_log:
        h_arr = np.logspace(args.h_min,args.h_max,num=args.h_log_num)
    else:
    ...
    if not load_matrices:
        ...
        if model in ["SK", "IR"]:
            if model == "IR":
                ...
                Jij = tfim.Jij_instance(N,J,dist="bimodal",seed=args.S)
    ...
    for h in bar(h_arr):
        ...
        if full_diag:
            ...
            # Sparse diagonalize
            E,v = spla.eigsh(H, k=k, which='SA', v0=v0, maxiter=100000)
        ...
        Delta = E - E[0]
        psi0_a = v[:,0]
        psi0_b = np.flip(psi0_a)
        psi0_sym = (psi0_a + psi0_b) / 2.0
        psi0_asym = (psi0_a - psi0_b) / 2.0
        
        if np.abs(np.dot(psi0,psi0_sym)) > np.abs(np.dot(psi0,psi0_asym)):
            psi0 = psi0_sym
        else:
            psi0 = psi0_asym
